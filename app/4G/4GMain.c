...
//	{XY_HY,		{116,62,125,35},8000, {"zj.fengjingit.com"} ,		1},
	//{XY_HY,		"81.69.35.239" ,		45513,   	2},
	
	{XY_YKC,	{121,43,69,62},8767,{0}	,		   	1},
	
	{XY_AP,		{114,55,186,206},5738, {0}  ,	 	1},
	
	{XY_XJ,	{139,199,242,25},9703, {0} ,	  		1},
	
	{XY_YL1,		{60,205,188,90},50000,{0} ,		1},
	
	{XY_YL2,		{47,108,227,112},8000,{0} ,	  	1},
	
	{XY_YL3,		{118,190,129,186},5042,{0} ,		1},
	
	{XY_YL4,		{81,70,61,150},50000,{0} ,		1},
	
	{XY_66,			{139,196,60,58},19040,{0} ,		1},
...
INT8U UART_4GWrite(INT8U* const FrameBuf, const INT16U FrameLen)
{
	...
	INT8U err;
	    ...
	    //读取信号强度不打印
		if(strncmp((char *)FrameBuf, "AT+CSQ",strlen("AT+CSQ")) != 0)
		{
			printf("SM4gT:");
			if((SYSSet.NetNum == 2)&&((APP_GetModuleConnectState(0) != STATE_OK) || (APP_GetModuleConnectState(1) != STATE_OK)))
			{
			...
			else if((SYSSet.NetNum == 1)&&(APP_GetModuleConnectState(0) != STATE_OK)) 
			{
				for(len = 0;len < FrameLen;len++)
				{
					printf("%c",FrameBuf[len]);
			...
			if(APP_GetModuleConnectState(0) == STATE_OK)
			{
				if(APP_GetSIM7600Mode() == MODE_DATA)
				{
					...
					if(strncmp((char *)FrameBuf, "AT+CIPSEND=1,",strlen("AT+CIPSEND=1,")) == 0)
					{
						//printf("1PTsend:%d\r\n",FrameBuf[strlen("AT+CIPSEND=1, ")]);
						printf("ZFPTsend");  //发送数据长度 
					}
					else if(strncmp((char *)FrameBuf, "AT+CIPSEND=0,",strlen("AT+CIPSEND=0,")) == 0)
					{
						//printf("0PTsend:%d\r\n",FrameBuf[strlen("AT+CIPSEND=0, ")]);
						printf("0PTsend");
					}
					else if(strncmp((char *)FrameBuf, "AT+CIPRXGET=2,0",strlen("AT+CIPRXGET=2,0")) == 0)
					{
						 printf("0PThuoqu\r\n");
					}else if(strncmp((char *)FrameBuf, "AT+CIPRXGET=2,1",strlen("AT+CIPRXGET=2,1")) == 0)
					{
						  printf("ZFPThuoqu\r\n");
					}
					else
...
//		if((APP_GetModuleConnectState(0) ==STATE_OK) && (APP_GetModuleConnectState(1) ==STATE_OK))   //双平台服务器先注册成功
//		{
//			if((APP_GetAppRegisterState(0) != STATE_OK) || (APP_GetAppRegisterState(1) != STATE_OK))  //然后再是协议注册再成功
//			{
//				if(++num22 >= (RESET_4G_COUNT*4))
//				{
//					num22 = 0;
//					printf("XYzhuce_weilianjie_0PT\r\n");  //这个是某一个平台注册不了
//					SIM7600Reset();
//				}
//			}
//			 else
...
//如果是IP端口不对或者服务器宕机，ZF平台不会回复，更不会注册成功，
//如果是IP端口正确，服务器没有宕机，桩编号错误也会注册成功的（前提是两个平台同时连接）
            if(APP_GetAppRegisterState(1) != STATE_OK)  
			{
				if(++num22 >= 1500)  //重联50次，大概1分钟时间 ---30分钟大概复位一次
				{
					num22 = 0;
					printf("ZFPTzhuce_weishou\r\n"); 
					//就是政府的平台注册未收到(第二个平台单独会不断的重联)，大概1-3分钟(上面的RESET_4G_COUNT*10，大概重新登录次数大概在6-7次，同理*20大概12-14次)，开始重启4G
 					SIM7600Reset();
				}
			}
			 else
