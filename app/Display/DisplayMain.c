...
extern _CCU_SEND_DATA CCUSendData[GUN_MAX];				//CCU·¢ËÍÊý¾Ý½á¹¹Ìå
...
_ERRWARN_RECODE_CONTROL ErrWarnRecodeControl;	//故障交易记录控制
_ERRWARN_RECODEINFO ErrWarningRecodInfo[GUN_MAX];					//故障记录
...
//=====堆充的交流接触器2024年5月7日=======
INT8U Dispose_ACSW(void)
{
	static INT32U lasttime[GUN_MAX];
	INT32U nowSysTime[GUN_MAX];

	nowSysTime[GUN_A] = OSTimeGet();
	nowSysTime[GUN_B] = OSTimeGet();
	if(GetGunState(GUN_B) == GUN_DISCONNECTED)
	{
		if((nowSysTime[GUN_B] >= lasttime[GUN_B]) ? ((nowSysTime[GUN_B] - lasttime[GUN_B]) >= SYS_DELAY_1M) : \
		        ((nowSysTime[GUN_B] + (0xFFFFFFFF - lasttime[GUN_B])) >= SYS_DELAY_1M))
		{
			lasttime[GUN_B] = nowSysTime[GUN_B];
			CCUSendData[GUN_B].SendHeart.ACSW = 0;
		}
	}
	else
	{
		...
		lasttime[GUN_B] = nowSysTime[GUN_B];
		CCUSendData[GUN_B].SendHeart.ACSW = 1;
	}
	...
	{
		if((nowSysTime[GUN_A] >= lasttime[GUN_A]) ? ((nowSysTime[GUN_A] - lasttime[GUN_A]) >= SYS_DELAY_1M) : \
		        ((nowSysTime[GUN_A] + (0xFFFFFFFF - lasttime[GUN_A])) >= SYS_DELAY_1M))
		{
			lasttime[GUN_A] = nowSysTime[GUN_A];
			CCUSendData[GUN_A].SendHeart.ACSW = 0;
		}
	}
	else
	{
		...
		lasttime[GUN_A] = nowSysTime[GUN_A];
		CCUSendData[GUN_A].SendHeart.ACSW = 1;
	}
}
...
static INT8U APP_RecvCardAnalyze(_BSP_MESSAGE *pMsg)
{
	...
	switch(pMsg->DivNum)
	{
		...
		case NETCARDCMD_STARTA:		  //A枪网络开始刷卡
			...
			if(SYSSet.NetYXSelct == XY_HY||  (SYSSet.NetYXSelct == XY_YL2)||(SYSSet.NetYXSelct == XY_TT)||  (SYSSet.NetYXSelct == XY_JG)|| (SYSSet.NetYXSelct == XY_YKC)|| (SYSSet.NetYXSelct == XY_YL3)|| (SYSSet.NetYXSelct == XY_YL4)|| (SYSSet.NetYXSelct == XY_XJ)||(SYSSet.NetYXSelct == XY_YL1))
			{
				if(DispControl.NetGunState[GUN_A] != GUN_IDLE)
				{
					DisplayCommonMenu(HYMenu22.Menu_PrePage,NULL);
		...
		case NETCARDCMD_STOPA:		 //A枪网络停止充电
			if((SYSSet.NetYXSelct == XY_AP) || (SYSSet.NetYXSelct == XY_HY)||(SYSSet.NetYXSelct == XY_TT)||  (SYSSet.NetYXSelct == XY_JG)||  (SYSSet.NetYXSelct == XY_YL2)|| (SYSSet.NetYXSelct == XY_YKC)|| (SYSSet.NetYXSelct == XY_YL3)|| (SYSSet.NetYXSelct == XY_YL4)|| (SYSSet.NetYXSelct == XY_XJ)||(SYSSet.NetYXSelct == XY_YL1))
			{
		...
		case NETCARDCMD_STOPB:		 //B枪网络停止充电
			if((SYSSet.NetYXSelct == XY_AP) || (SYSSet.NetYXSelct == XY_HY)||  (SYSSet.NetYXSelct == XY_JG)||  (SYSSet.NetYXSelct == XY_YL2)|| (SYSSet.NetYXSelct == XY_YKC)||(SYSSet.NetYXSelct == XY_TT)|| (SYSSet.NetYXSelct == XY_YL3)|| (SYSSet.NetYXSelct == XY_YL4)|| (SYSSet.NetYXSelct == XY_XJ)||(SYSSet.NetYXSelct == XY_YL1))
			{
				if((FRAMERTU)(*pMsg->pData) == _SuccessRtu)
				{
...
void Display_ShowSet1(void)
{
	...
	sys_set1.UseGun = ((SYSSet.PMUNUM+1) << 8);
...
static void Display_ParaInit(void)
{
	...
	static INT8U flash_flag = 0;   //写入flash标志位  0不写入，1写入flash
	//枪号空 + 枪号不一样 + 电表号空
	if((SYSSet.SysSetNum.UseGun == 0XFF) || (SYSSet.SysSetNum.UseGun != USER_GUN+1) || (SYSSet.MeterAdd[GUN_A][0] == 0xFF) )
	{
		if(USER_GUN == USER_SINGLE_GUN)
		{
			SYSSet.SysSetNum.UseGun = 1;  //单
			memcpy(SYSSet.MeterAdd[GUN_A],MeterAdd,12);
			memset(SYSSet.MeterAdd[GUN_B],0,sizeof(MeterAddB[12])); //第二个表清零
		}
		else if(USER_GUN == USER_DOUBLE_GUN)
		{
			SYSSet.SysSetNum.UseGun = 2;  //双
			memcpy(SYSSet.MeterAdd[GUN_A],MeterAddA,12);
			memcpy(SYSSet.MeterAdd[GUN_B],MeterAddB,12);
		}
		else
		{
			SYSSet.SysSetNum.UseGun = 0xFF;
		}
		flash_flag = 1;
	}

	//协议选择    网络个数最多2个
	if((SYSSet.NetYXSelct ==0xFF)||(SYSSet.NetYXSelct >= XY_MAX)||(SYSSet.NetNum ==0xFF)||(SYSSet.NetNum > 2))
	{
		SYSSet.NetYXSelct = XY_HY;  //默认HY协议
		memcpy(SYSSet.IP,NetConfigInfo[SYSSet.NetYXSelct].IP,sizeof(SYSSet.IP));
	...
	if((SYSSet.LockState == 0xff) || (SYSSet.CurModule == 0xff) ||((SYSSet.NetState != DISP_NET) && (SYSSet.NetState != DISP_CARD)))
	{
		SYSSet.NetState = DISP_CARD;  //默认单机
		SYSSet.LockState = 1;        //常闭
		SYSSet.CurModule = 0;       //永联恒功率
		SYSSet.GunTemp = 0;       //枪温0开启  1关闭
		flash_flag = 1;
	}


	if((SYSSet.SOCthreshold < 90)||(SYSSet.SOCthreshold >100))
	{
		SYSSet.SOCthreshold = 98;  //SOC
		flash_flag = 1;
	}
	if(flash_flag == 1)  //是否写入flash
	{
		flash_flag = 0;  //标志位清空
		FlashOper.DataID = PARA_CFGALL_ID;
		FlashOper.Len = MIN(PARA_CFGALL_FLLEN,sizeof(_SYS_SET) );
	...
	if((SYSSet.NetYXSelct == XY_YKC)||(SYSSet.NetYXSelct == XY_TT)|| (SYSSet.NetYXSelct == XY_YL3)|| (SYSSet.NetYXSelct == XY_YL4)|| (SYSSet.NetYXSelct == XY_XJ)||(SYSSet.NetYXSelct == XY_YL1))
	{
		//云快充  当充电中修改费率，A枪的费率可能和B枪的费率不一样
	...
	if((PresetValue.Therold_Value.OverACVolTherold != AC_POWER_OVER_VAL) || (PresetValue.Therold_Value.UnderACVolTherold != AC_POWER_UNDER_VAL))
	{
		PresetValue.Therold_Value.OverACVolTherold=AC_POWER_OVER_VAL;    //交流输入过压阀值=450V
		PresetValue.Therold_Value.UnderACVolTherold=AC_POWER_UNDER_VAL;  //交流输入欠压阀值=330V
		flash_flag = 1;
	}
		
	if((PresetValue.VolMaxOutput < MAX_VOL_OUTPUT)||(PresetValue.VolMaxOutput > 10000)|| \
		(PresetValue.Therold_Value.OverOutVolTherold < 7600)||(PresetValue.Therold_Value.OverOutVolTherold > 10500))
	{
		PresetValue.VolMaxOutput = MAX_VOL_OUTPUT;      //最大输出电压=750V
		PresetValue.Therold_Value.OverOutVolTherold = MAX_VOL_OUTPUT + 150;  // x<7600或 x>1050 输出电压阀值==750V+15V
		flash_flag = 1;
	}
	
	if((PresetValue.CurMaxOutput < 100)||(PresetValue.CurMaxOutput > 2500)|| \
		(PresetValue.Therold_Value.OverOutCurTherold < 200)||(PresetValue.Therold_Value.OverOutCurTherold > 2600))
	{
		PresetValue.CurMaxOutput = MAX_CUR_OUTPUT;  //最大输出电流=120A
		PresetValue.Therold_Value.OverOutCurTherold = MAX_CUR_OUTPUT+100;  //空、x<200或 x>2600  //最大输出电流阀值= MAX_CUR_OUTPUT+10A
		flash_flag = 1;
	}

	if(PresetValue.VolMinOutput != MIN_VOL_OUTPUT)
	{
		PresetValue.VolMinOutput=MIN_VOL_OUTPUT;  //最小输出电压为150V
		flash_flag = 1;
	}
	
	if((PresetValue.PowerModelNumber==0xFF)||(PresetValue.PowerModelNumber==0))
	{
		PresetValue.PowerModelNumber=MODULE_MAX_NUM;  //模块数量-最大8个
		flash_flag = 1;
	}
	if((PresetValue.ChargeType==0xFF)||(PresetValue.ChargeType==0))
	{
		PresetValue.ChargeType=0x01;  //充电方式
		flash_flag = 1;
	}
...
//	{
	//进口隔离芯片
//		ADCJustInfo[GUN_A].BatVoltKB.line_X1 = 1225;
//		ADCJustInfo[GUN_A].BatVoltKB.line_X2 = 1675;
//		ADCJustInfo[GUN_A].BatVoltKB.line_Y1 = 30000;
//		ADCJustInfo[GUN_A].BatVoltKB.line_Y2 = 50000;
//		ADCJustInfo[GUN_A].BatVoltKB.line_K = (FP32)(ADCJustInfo[GUN_A].BatVoltKB.line_Y2-ADCJustInfo[GUN_A].BatVoltKB.line_Y1)/ (FP32)(ADCJustInfo[GUN_A].BatVoltKB.line_X2-ADCJustInfo[GUN_A].BatVoltKB.line_X1);
//		ADCJustInfo[GUN_A].BatVoltKB.line_B = (FP32)ADCJustInfo[GUN_A].BatVoltKB.line_Y2 -  (FP32)ADCJustInfo[GUN_A].BatVoltKB.line_K* (FP32)ADCJustInfo[GUN_A].BatVoltKB.line_X2;
//		ADCJustInfo[GUN_A].HeadOutKB.line_X1 = 1065;
//		ADCJustInfo[GUN_A].HeadOutKB.line_X2 = 1605;
//		ADCJustInfo[GUN_A].HeadOutKB.line_Y1 = 400;
//		ADCJustInfo[GUN_A].HeadOutKB.line_Y2 = 600;
//		ADCJustInfo[GUN_A].HeadOutKB.line_K = (FP32)(ADCJustInfo[GUN_A].HeadOutKB.line_Y2-ADCJustInfo[GUN_A].HeadOutKB.line_Y1)/ (FP32)(ADCJustInfo[GUN_A].HeadOutKB.line_X2-ADCJustInfo[GUN_A].HeadOutKB.line_X1);
//		ADCJustInfo[GUN_A].HeadOutKB.line_B = (FP32)ADCJustInfo[GUN_A].HeadOutKB.line_Y2 -  (FP32)ADCJustInfo[GUN_A].HeadOutKB.line_K* (FP32)ADCJustInfo[GUN_A].HeadOutKB.line_X2;
//		ADCJustInfo[GUN_A].JYVolKB.line_X1 = 517;
...
static INT8U DispShow_State(INT32U time)
{
	...
	for(i = 0; i < num; i++)
	{
		...
		if((APP_GetErrState((_GUN_NUM)i) == 0x04) || (GetMeterStatus((_GUN_NUM)i) == _COM_FAILED)) 				//有故障
		{
			...
			else if( BCURemoteSignal[i].state1.State.PCU_Link == 0x01)
			{
				show_err[(_GUN_NUM)i] = SHOW_SYS_PCUTIMEOUT; //与PCU通信超时
			}
			else if(SysState[i].GUNIDState == GUN_ID_FAIL)
			{
				show_err[(_GUN_NUM)i] = SHOW_SYS_IDFAIL;  //ID重复
			}
			else if(CCURecvData[i].RecvHeart.ifGunUser)
			{
				show_err[(_GUN_NUM)i] = SHOW_GUN_NOTUSER;  //充电枪不可用
			}
			else if(CCURecvData[i].RecvHeart.PDUSingnal)
			{
				show_err[(_GUN_NUM)i] = SHOW_PDU_SINGNAL;  //PDU通信超时
			}
			else {
		...
		if(++count[i] >= (SYS_DELAY_1s/time) )
		{
			...
			if((show_err[(_GUN_NUM)i] == SHOW_SYS_EMERGENCY) || (show_err[(_GUN_NUM)i] == SHOW_SYS_PCUTIMEOUT)  || (show_err[(_GUN_NUM)i] == SHOW_SYS_IDFAIL) \
			        || (show_err[(_GUN_NUM)i] == SHOW_PDU_SINGNAL))
			{
				//系统故障显示
				if(cnt[GUN_A] == 0)
				{
	...
	if(((SYSSet.NetYXSelct == XY_YKC) ||(SYSSet.NetYXSelct == XY_TT)|| (SYSSet.NetYXSelct == XY_YL3)|| (SYSSet.NetYXSelct == XY_YL4)|| (SYSSet.NetYXSelct == XY_XJ)||(SYSSet.NetYXSelct == XY_YL1))  && (SYSSet.NetState == DISP_NET))
	{
		for(i = 0; i < GUN_MAX; i++)
		{
...
{
	INT8U buf[100]  = {0};
	char * net = "https://api.huichongchongdian.com/cpile/coffee/";  //汇誉
	...
	//char * ykcnet = "http://www.ykccn.com/MPAGE/index.html?pNum=";   //云快充
	char * ykcnet = "http://wx.evking.cn/app/download?orgNo=MA2GK1XG8&code=";  //蔚景云
...
static void NetCardDispose(INT32U time)
{
	...
	if((SYSSet.NetYXSelct != XY_AP) && (SYSSet.NetYXSelct != XY_HY)&& (SYSSet.NetYXSelct != XY_JG)&&(SYSSet.NetYXSelct != XY_YL2) \
	        && (SYSSet.NetYXSelct != XY_YKC)&& (SYSSet.NetYXSelct != XY_TT)&& (SYSSet.NetYXSelct != XY_YL3)&& (SYSSet.NetYXSelct != XY_YL4) && ((SYSSet.NetYXSelct != XY_XJ)) \
...
static void DispPeriod(void)
{
	...
	{
		lastMenu =CurMenu;
		PrintStr(0x0100,(INT8U *)SYS_APP_VER,strlen(SYS_APP_VER));   //显示状态
	}
	if(SYSSet.NetState == DISP_NET)
	{
