...
_POWER_MODULE_INFO 	PowerModuleInfo[GUN_MAX] = {0}; 	//模块整体信息
extern _CCU_SEND_DATA CCUSendData[GUN_MAX];				//CCU发送数据结构体
...
{    
   ...
   {//枪未连接清状态	
	...
	Vol_Judge = GetRM_DCOutVolt(gun) /10;			//用模块电压 容易报E14故障   20240306
...
void APP_DCSWDispose(INT8U gun)
{
	...
	if((CCURecvData[gun].RecvHeart.PDUSingnal  ==  GUN_CLOSE)  && (GetRM_DCOutVolt(gun) > 20000) )
	{
		...
		((curtime[gun] + (INT32U_MAX_NUM - lasttime[gun])) >= SYS_DELAY_15s ) )
		{
			...
			CCUSendData[gun].SendHeart.DCSWFaile  = 1;						//直流接触器黏连，相应的枪不可用
		}
	}
	else
	{
...
void TaskPeriod(void *data)
{
    ...
    while (1)
    {
		...
		for(gun = 0;gun < GUN_MAX;gun++)
		{
			...
			{
				BSP_IOOpen(IO_RLY_FAN);  //打开风扇
			}
			else
			{
				BSP_IOClose(IO_RLY_FAN);  //关闭风扇
			}
			//12、未充电，充电桩后级有电压，需要关闭相应的枪	还需要关闭相应的枪！3把枪 能关闭2把枪 另外一把枪怎么关闭?
			//可能存在一插枪后级就有电压
			//APP_DCSWDispose(gun);
		}
    }
