...
#define TASK_STK_SIZE_PCUMAIN  0x800
OS_STK TaskConnectPCUMAINStk[TASK_STK_SIZE_PCUMAIN]; // PCU对接主函数
...
#define	TASK_STK_SIZE_MODULER		0x800
OS_STK TaskModuleRStk[TASK_STK_SIZE_MODULER];					//模块接收任务

#define	TASK_STK_SIZE_MODULET		0x800
OS_STK TaskModuleTStk[TASK_STK_SIZE_MODULET];					//模块发送任务
OS_STK TaskUsbMainStk[TASK_STK_SIZE_MODULET];					//模块发送任务
OS_STK TaskUsbStk[TASK_STK_SIZE_MODULET];					//模块发送任务

#define	TASK_STK_SIZE_PCUCONTROL		0x800
OS_STK TaskPCUControlTStk[TASK_STK_SIZE_PCUCONTROL];			//PCu控制任务

#define	TASK_STK_SIZE_MODULEPRIOD		0x800
OS_STK TaskModulePriodTStk[TASK_STK_SIZE_MODULEPRIOD];			//模块周期性任务
...
//				//模块故障
    ...
    OSTaskCreateExt(TaskADJust,
			 ...
			 //创建任务,PCU控制任务了
    OSTaskCreateExt(TaskPCUTxRxMain,
                    (void *)0,
                    &TaskPCUControlTStk[TASK_STK_SIZE_PCUCONTROL - 1],
                    PRI_PCU_CONTROL,
                    PRI_PCU_CONTROL,
                    &TaskPCUControlTStk[0],
                    TASK_STK_SIZE_PCUCONTROL,
                    (void *)0,
                    OS_TASK_OPT_STK_CHK + OS_TASK_OPT_STK_CLR);		

			//模块周期性任务
		OSTaskCreateExt(TaskPCUTxMain,
	//OSTaskCreateExt(TaskPCUControl,
                    (void *)0,
                    &TaskModulePriodTStk[TASK_STK_SIZE_MODULEPRIOD - 1],
                    PRI_MODULE_PRIOD,
                    PRI_MODULE_PRIOD,
                    &TaskModulePriodTStk[0],
                    TASK_STK_SIZE_MODULEPRIOD,
                    (void *)0,
                    OS_TASK_OPT_STK_CHK + OS_TASK_OPT_STK_CLR);
    ...
    OSTaskCreateExt(TaskUsbTask,
                    (void *)0,
                    &TaskUsbStk[TASK_STK_SIZE_MODULET - 1],
                    PRI_USBTASK,
                    PRI_USBTASK,
                    &TaskUsbStk[0],
                    TASK_STK_SIZE_MODULET,
	...
	IWDG_Config(IWDG_Prescaler_64 ,625*12);	  //6s		
					
	while(1)
	{	
		BSP_IOTurnover(IO_LED_RUN);
		OSTimeDly(SYS_DELAY_500ms); 
		IWDG_ReloadCounter();			//喂狗   充电枪故障 输出电压异常 泄放回路故障
	}
